...
def main() -> int:
    ...
    ax.set_title(f"Network {args.metric.upper()} over mission time{title_suffix}")
    ax.grid(alpha=0.3)
    ax.legend(loc="best")
    fig.tight_layout()
    ...
    plt.close(fig)
    pd.DataFrame(summary_rows).to_csv(out_dir / "network_timeseries_summary.csv", index=False)
    print(f"Saved: {png_path}")
    print(f"Saved: {pdf_path}")
    print(f"Saved: {svg_path}")
    print(f"Saved: {out_dir / 'network_timeseries_summary.csv'}")
    return 0
