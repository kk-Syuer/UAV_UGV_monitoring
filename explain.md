...
- X 轴：`Mission time (s)`，单位是秒（s）
  - 由脚本根据同策略 `status_timeseries.csv` 的任务起始时间进行对齐（`death_events.csv` 不再按自身首个死亡事件归一化），因此不同协议“第一次死亡出现得早/晚”会被保留。
- Y 轴：`Cumulative death events (count)`，单位是次数（count）
  - 含义：到当前时间累计发生了多少次死亡事件。

### 为什么现在都从 0 开始？
脚本已经强制了基线点 `(t=0, y=0)`，并把真实死亡时刻轻微右移一个极小量（`EPS=1e-6`）来避免“在 t=0 同时出现 0 和 1 导致非零起点”的绘图伪影。
